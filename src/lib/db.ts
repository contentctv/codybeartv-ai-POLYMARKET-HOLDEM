import { DeskSealedError } from "./errors";
...
export function getSql(): Promise<Sql> {
  sqlPromise ??= createSql().catch((err) => {
    ...
    throw new DeskSealedError(
      err instanceof Error ? err.message : "Desk sealed — database unavailable",
    );
  });
  return sqlPromise;
}
...
if (typeof window === "undefined" && dbSource === "pglite") {
  globalBoot.__pgBootstrapPromise__ ??= ensureDbReady().catch((err) => {
    ...
    console.error("[db] PGLite bootstrap failed — fail closed per request:", err);
  });
}
